---
title: "Heterogeneous Treatment Effects of Intensive Glycemic Control on Kidney Microvascular Outcomes and Mortality in ACCORD: baseline UACR"
...
We define UACR quartiles and plot the distribution within treatment arms. 
...
# UACR quartile thresholds
uacr_quart_thresh = quantile(df$uacr, seq(0.25, 0.75, by = 0.25))
# Create groups based on thresholds
df$uacr_quarts = as.numeric(cut(df$uacr, 
                                breaks = c(-Inf, as.numeric(uacr_quart_thresh), Inf),
                                labels = c(1:(length(uacr_quart_thresh)+1))))

# Histograms of UACR for treatment and control
df %>% 
  ggplot(aes(x = uacr, fill = as.factor(glyarm), color = as.factor(glyarm))) + 
  geom_histogram(alpha = 0.3, position="identity") + 
  scale_x_log10() +
  geom_vline(xintercept = uacr_quart_thresh, color = "gray60") + 
  geom_text(data = data.frame(uacr_quart_thresh = uacr_quart_thresh), 
             aes(x = uacr_quart_thresh, y = Inf, 
                 label = round(uacr_quart_thresh, 3)), 
             inherit.aes = FALSE, color = "grey60", 
            hjust = 1.1, vjust = 1.4, size = 3, angle = 90) + 
  xlab("5-year predicted risk by UACR (log)") + ylab("") + 
  scale_fill_discrete(name = "", labels = c("0" = "Control", "1" = "Treatment")) + 
  scale_color_discrete(name = "", labels = c("0" = "Control", "1" = "Treatment"))
ggsave("figs_and_tabs/uacr_hist.png", width = 6, height = 4)

# Distribution of UACR for treatment and control
uacr_dist_tab = data.frame(
  do.call(rbind, c(list(summary(df$uacr)), 
                 tapply(df$uacr, df$glyarm, summary))), 
  row.names = c("Overall", "Control", "Treatment")
)
names(uacr_dist_tab) = c("Min", "Q1", "Median", "Mean", "Q3", "Max")
write.csv(uacr_dist_tab %>% round(5), file = "figs_and_tabs/uacr_dist_tab.csv")
uacr_dist_tab %>% round(5)
```

These are summary statistics for the scores within each UACR quartile. 
...
# Summary stats for UACR within quartile
uacr_quart_tab = data.frame(do.call(rbind, 
        lapply(1:4, function(i) {
          c(Quartile = i, 
            my_summary(df$uacr[df$uacr_quarts==i]))
        })
))
write.csv(uacr_quart_tab %>% round(5), 
          file = "figs_and_tabs/uacr_quart_tab.csv", row.names = FALSE)
uacr_quart_tab %>% round(5)
```

Median and IQR of baseline eGFR within each KFRE and baseline eGFR quartile. 

```{r}
kfre_uacr_cross_tab = sapply(1:4, function(i) {
  out = sapply(1:4, function(j) {
    x = df$uacr[df$kfre_quarts==i & df$uacr_quarts==j]
    paste0(round(median(x, na.rm = TRUE), 2), " (", 
           round(IQR(x, na.rm = TRUE), 2),  ")")
  })
  names(out) = paste0("Baseline UACR Q", 1:4)
  return(out)
}) %>% t() %>% 
  data.frame(row.names = paste0("KFRE Q", 1:4)) %>% 
  set_names(paste0("Baseline UACR Q", 1:4))

write.csv(kfre_uacr_cross_tab, 
          file = "figs_and_tabs/kfre_uacr_cross_tab.csv")
kfre_uacr_cross_tab
```

Number of people in the KFRE and baseline UACR quartiles. 

```{r}
table(KFRE = df$kfre_quarts, "Baseline UACR" = df$uacr_quarts)
```

## Treatment effects

We estimate the 7-year RMST differences for each subgroup defined by the UACR quartiles. 
...
overall_rmst_fits = lapply(outcome_list, function(x){
  df_sub = na.omit(df[,c(x["time"], x["status"], "glyarm", "uacr_quarts")])
  rmst2(df_sub[,x["time"]], 
        df_sub[,x["status"]], 
        df_sub[,"glyarm"], 
        tau = horizon)
...
uacr_quart_rmst_fits = lapply(outcome_list, function(x){
  df_sub = na.omit(df[,c(x["time"], x["status"], "glyarm", "uacr_quarts")])
  lapply(1:4, function(i){
    rmst2(df_sub[df_sub$uacr_quarts==i,x["time"]], 
          df_sub[df_sub$uacr_quarts==i,x["status"]], 
          df_sub[df_sub$uacr_quarts==i,"glyarm"], 
          tau = horizon)
  })
})
names(uacr_quart_rmst_fits) = names(outcome_list)
uacr_quart_rmst = sapply(names(outcome_list), function(outcome){
  sapply(1:4, function(i){
    uacr_quart_rmst_fits[[outcome]][[i]]$unadjusted.result[1,1]
  })
})
```
...
```{r, echo = FALSE}
load("boot_uacr_rmst.rData")
```

```{r, eval = FALSE}
boot_uacr_rmst = foreach::foreach(
  b = 1:B, 
  .packages = c("survRM2")
  ) %dopar% {
    # Set seed
    ...
    overall_rmst = sapply(outcome_list, function(x){
      df_sub = na.omit(df_boot[,c(x["time"], x["status"], "glyarm", "uacr_quarts")])
      rmst2(df_sub[,x["time"]], 
            df_sub[,x["status"]], 
            df_sub[,"glyarm"], 
    ...
    uacr_quart_rmst = sapply(outcome_list, function(x){
      df_sub = na.omit(df_boot[,c(x["time"], x["status"], "glyarm", "uacr_quarts")])
      sapply(1:4, function(i){
        rmst2(df_sub[df_sub$uacr_quarts==i,x["time"]], 
              df_sub[df_sub$uacr_quarts==i,x["status"]], 
    ...
    return(list(overall_rmst = overall_rmst, 
                uacr_quart_rmst = uacr_quart_rmst))
  }

save(boot_uacr_rmst, file = "boot_uacr_rmst.rData")
```
...
# Overall
uacr_overall_rmst_boot_ci = apply(
  sapply(boot_uacr_rmst, function(x){
    x$overall_rmst
  }), 1, quantile, c(0.025, 0.975))

# Not normalized
uacr_quart_rmst_boot_ci = lapply(1:4, function(i){
  dat = sapply(boot_uacr_rmst, function(x){
    x$uacr_quart_rmst[i,]
  })
...
# Make tables
uacr_quart_rmst_boot_tabs = lapply(names(outcome_list), function(outcome){
  out = rbind(
    data.frame(Quartile = "Overall", 
               "Est" = overall_rmst[outcome], 
               t(uacr_overall_rmst_boot_ci[,outcome])), 
    data.frame(Quartile = 1:4, 
               "Est" = uacr_quart_rmst[,outcome], 
               t(sapply(uacr_quart_rmst_boot_ci, function(x){x[,outcome]}))))
  rownames(out) = NULL
  return(out)
})
names(uacr_quart_rmst_boot_tabs) = names(outcome_list)

# Format table for printing
rmst_tab = data.frame(
  Outcome = rep(names(uacr_quart_rmst_boot_tabs), 
                each = nrow(uacr_quart_rmst_boot_tabs[[1]])), 
  do.call(rbind, uacr_quart_rmst_boot_tabs), 
  row.names = NULL
)
...
uacr_quart_rmst_norm_boot_ci = lapply(1:4, function(i){
  dat = sapply(boot_uacr_rmst, function(x){
    x$uacr_quart_rmst[i,] - x$overall_rmst
  })
  apply(dat, 1, quantile, c(0.025, 0.975))
})

# Make tables
uacr_quart_rmst_boot_norm_tabs = lapply(names(outcome_list), function(outcome){
  data.frame(Quartile = 1:4, 
               "Est" = uacr_quart_rmst[,outcome] - overall_rmst[outcome], 
               t(sapply(uacr_quart_rmst_norm_boot_ci, function(x){x[,outcome]})))
})
names(uacr_quart_rmst_boot_norm_tabs) = names(outcome_list)

# Format table for printing
rmst_norm_tab = data.frame(
  Outcome = rep(names(uacr_quart_rmst_boot_norm_tabs), 
                each = nrow(uacr_quart_rmst_boot_norm_tabs[[1]])), 
  do.call(rbind, uacr_quart_rmst_boot_norm_tabs), 
  row.names = NULL
)
...
```{r}
for (outcome in names(uacr_quart_rmst_boot_tabs)) {
  ate_tab = uacr_quart_rmst_boot_tabs[[outcome]][1,]
  p1 = uacr_quart_rmst_boot_tabs[[outcome]][-1,] %>% 
    mutate(x = 1:4) %>% 
...
```{r}
for (outcome in names(uacr_quart_rmst_boot_norm_tabs)) {
  p1 = uacr_quart_rmst_boot_norm_tabs[[outcome]] %>% 
    mutate(x = 1:4) %>% 
    ggplot(aes(x = x, y = as.numeric(Est))) +
    geom_pointrange(aes(ymin = X2.5., ymax = X97.5.), size = 0.25) + 
    scale_x_continuous(breaks = 1:4, labels = 1:4) + 
...
# Include spline at 4 months
fit_egfr_uacr_quarts = lapply(1:4, function(i) {
  lme(egfr ~ days_from_baseline_165 * treat + spline * treat, 
      data = egfr_df %>% filter(uacr_quarts == i), 
      random = ~ 1 + spline + spline:treat | MaskID, 
      control = lmeControl(opt = "optim", optimMethod = "L-BFGS-B"))
})

# Save model fits
save(fit_egfr, fit_egfr_uacr_quarts, file = "fit_egfr.rData")
```
...
coef_egfr = do.call(rbind, lapply(1:4, function(i) { 
  as.data.frame(cbind(uacr_quarts = i, coef(fit_egfr_uacr_quarts[[i]])))
}))
...
baseline_uacr_quart_egfr = sapply(1:4, function(i) {
  df_sub = df[df$uacr_quarts == i,]
...
n_baseline_uacr_quarts = as.numeric(table(df$uacr_quarts))
# Fixed effect estimates
fixed_eff = lapply(fit_egfr_uacr_quarts, fixed.effects)
# Fixed and random covariance matrices
fixed_cov_mat = lapply(fit_egfr_uacr_quarts, vcov)
random_cov_mat = lapply(fit_egfr_uacr_quarts, getVarCov)

boot_baseline_uacr_egfr = foreach::foreach(
  b = 1:B, .packages = c("tidyverse", "mvnfast")
    ...
    # Calculate eGFR slope difference for each quartile
    baseline_uacr_quart_egfr = tapply(slopes_by_quart$egfr_slope, 
                                      slopes_by_quart$baseline_uacr_quarts, mean)
    
    # Slope for each treatment arm  and quartile (used for plotting)
    egfr_lm_coefs = rbind(slopes_by_quart %>% mutate(baseline_uacr_quarts = "Overall"), 
                          slopes_by_quart) %>% 
      mutate(Quartile = factor(baseline_uacr_quarts, levels = c("Overall", 1:4))) %>% 
      group_by(Quartile) %>% 
      summarize(
    ...
    return(list(overall_egfr = overall_egfr, 
                baseline_uacr_quart_egfr = baseline_uacr_quart_egfr, 
...
```{r}
# Overall
baseline_uacr_overall_egfr_boot_ci = quantile(
  sapply(boot_baseline_uacr_egfr, function(x){
    x$overall_egfr
  }), c(0.025, 0.975))

# Not normalized
baseline_uacr_quart_egfr_boot_ci = sapply(1:4, function(i){
  quantile(sapply(boot_baseline_uacr_egfr, function(x){
    x$baseline_uacr_quart_egfr[i]
  }), c(0.025, 0.975))
})

# Make tables
baseline_uacr_quart_egfr_boot_tabs = rbind(
    data.frame(Quartile = "Overall", 
               "Est" = overall_egfr, 
               t(baseline_uacr_overall_egfr_boot_ci)), 
    data.frame(Quartile = 1:4, 
               "Est" = baseline_uacr_quart_egfr, 
               t(baseline_uacr_quart_egfr_boot_ci)))

# Format table for printing
egfr_tab = baseline_uacr_quart_egfr_boot_tabs
...
baseline_uacr_quart_egfr_norm_boot_ci = sapply(1:4, function(i){
  quantile(sapply(boot_baseline_uacr_egfr, function(x){
    x$baseline_uacr_quart_egfr[i] - x$overall_egfr
  }), c(0.025, 0.975))
})

# Make tables
baseline_uacr_quart_egfr_boot_norm_tabs = 
  data.frame(Quartile = 1:4, 
               "Est" = baseline_uacr_quart_egfr - overall_egfr, 
               t(baseline_uacr_quart_egfr_norm_boot_ci))

# Format table for printing
egfr_norm_tab = baseline_uacr_quart_egfr_boot_norm_tabs
...
```{r}
baseline_uacr_quart_egfr_boot_tabs[-1,] %>% 
  mutate(x = 1:4) %>% 
  ggplot(aes(x = x, y = as.numeric(Est))) +
  geom_pointrange(aes(ymin = X2.5., ymax = X97.5.), size = 0.25) + 
  geom_hline(data = baseline_uacr_quart_egfr_boot_tabs[1,], aes(yintercept = Est)) + 
  geom_rect(data = baseline_uacr_quart_egfr_boot_tabs[1,], 
            aes(x = NULL,y = NULL, 
                xmin = -Inf, xmax = Inf, 
